...
Como exemplo, o seguinte código para a instalação do pacote e utilizar seu aplicativo:
...
Para mais referências sobre como instalar, segue o [link do tutorial de instalação](http://shiny.rstudio.com/articles/shinyapps.html) para auxiliar na instalação.
...
## MIC-04 Experiências interativas em Ciência de Dados
...
Definiu-se *Potencial de Membrana* como: **Diferença em voltagem entre o interior e o exterior da membrana celular** 
...
## MIC-07 Ferramentas de estatística espacial no R para iniciantes

Toda observação possui uma referência **temporal** e **espacial**. Apesar de muitos estudos não farezem o uso da informação espacial, essas referências podem ser importantes para a análise.

A **Estatística espacial** é um cojunto de métodos de análise de dados em que a localização geográfica é usada explicitamente na análise.
...
Tipo de dados:
  ...
  * Padrão de pontos 
      * As localizações estão associadas à ocorrência de um fenômeno de interesse (conhecido) e são aleatórios (desconhecidas)
      * Objetivo investigar se os eventos observados exibem algum padrão sistemático (regularidade ou agrupamento)
  * Dados de área
      * Dados agregados em subregiões que totalizam uma dada área de interesse
      * Objetivo de investigar se as subregiões apresentam padrão aleatório, agrupamentos de área, explorar e determinar fatores determinantes dos padrões identificados.
  * Dados de superfícies contínuas (geoestatísticas)
      * Observações de um processo contínuo no espaço, coletadas em um conjunto finito de localizações
      * Objetivo de inferir sobre o processo gerados desses dados e fazer previsões em localidades não observadas.
  
### Sistema de informação Geográfica (SIG) e R

Um Sistema de Informação Geográfica (SIG ou GIS - Geographic Information System, do inglês) é um sistema de hardware, software, com procedimentos computacionais que permitem e facilitam a análise, gestão ou representação do espaço e dos fenômenos que nele ocorrem, como por exemplo Google Earth.

Existe a possibilidade de combinar a potencialidade do GIS como R, a seguir pode-se conferir algumas funcionalidades:

> library(rgdal)

O shapefile com os municípios do Rio de Janeiro podem ser baixados atráves deste [link do IBGE](https://downloads.ibge.gov.br/downloads_geociencias.htm) pelo caminho:  organizacao_do_territorio/malhas_territoriais/malhas_municipais/municipio_2015/UFs/RJ/rj_municipios.zip

 * Função readOGR() faz a leitura do arquivo shapefile, veja:
...
rio = readOGR("C:/Users/felipe.carvalho/Desktop/Apresentacao/rj_municipios/33MUE250GC_SIR.shp")
```

  * Função plot(): faz o plot do arquivo shapefile.

```{r}
## Plotando o shapefile com todos os municípios do estado do Rio de Janeiro
plot(rio)
```

Após alguns tratamentos, podemos utilizar o mapa de diversas maneiras, por exemplo:

> library(RColorBrewer) e library(sp)

![](C:/Users/felipe.carvalho/Desktop/Apresentacao/mapas1.png)
...
Para mais informações sobre o pacote, consultar o  [link com uma introdução ao uso do pacote](http://www2.uaem.mx/r-mirror/web/packages/plotGoogleMaps/vignettes/plotGoogleMaps-intro.pdf)

Exemplo de mapa:

![](C:/Users/felipe.carvalho/Desktop/Apresentacao/mapas2.png)

Mais algumas funcionalidades foram apresentadas no minicurso, para mais informações sobre como fazer as análises de estatística espacial podem ser conferidas o [link do minicurso](http://www.semest.uff.br/images/meusarquivos/MatApr2017/MIC-07-JonyArrais.pdf)
...
Além das palestras dos visitantes, um dos alunos da universidade fez parte do desenvolvimento de um novo pacote para o R chamado [BETS](https://cran.r-project.org/web/packages/BETS/index.html), desenvolvido pelo Fundo de Pesquisa Aplicada da FGV, mais informações sobre o pacote podem ser conferidas no  [link da apresentação](http://slides.com/johnazevedo/deck-5/fullscreen#/).

**BETS** significa **Brazilian Economic Time Series**, e fornece de maneira descomplicada, as mais relevantes séries temporais ecônomicas coletadas de instituições como IBGE, Banco Central do Brasil e FGV IBRE e diversas ferramentas para analisá-las.

Para utilizá-lo, basta chamar o pacote com os seguintes comandos:
...
Entre suas funcionalidades, é possível realizar relatórios dinâmicos, gerenciamentos de dados, dashboards, dentre outras

### Gerenciamento de Dados

Para o gerenciamento de dados, algumas ferramentas para obter as séries e informações sobre elas da base de dados do pacote, veja:

```{r, eval=F}
#Exemplos de extração das séries temporais da base de dados do pacote:
...
São gerados automaticamente, bastando que o usuário forneça o código da série nas bases do BETS e alguns parâmetros adicionais, análises disponíveis incluem modelos de *Box & Jenkins* e *Redes neurais do tipo GRNN*, veja um exemplo:

![](C:/Users/felipe.carvalho/Desktop/Apresentacao/BETS1.png)


### Dashboards

Produção de gráficos e tabelas estilizados de séries históricas do brasil

![](C:/Users/felipe.carvalho/Desktop/Apresentacao/BETS2.png)


Na [documentação do BETS](https://github.com/nmecsys/BETS) é possível encontrar exemplos e demonstrações do uso de todas as funções do pacote.