...
![Note que essa aplicação funciona para qualquer pacote do R](imagem1.png)
...
```{r}
nomes=c('Sexo', 'Aniversário', 'Situação', 'Raça', 'Peso', 'Altura', 'IMC', 'Tipo físico', 'Tabagismo', 'tabaco por dia (cig/dia)', 'Alcool (dose/semana)', 'Drogas/g', 'Café/dia', 'Suco/dia', 'Obesidade', 'Magro');nomes
```
...
Substituir todos os f por i:

```{r}
exemplo2 <- str_replace_all(exemplo2, "f", "i") 
exemplo2
```
...
A manipulação de strigs é uma tarefa bem trabalhosa e algumas vezes até complexa, caso precise de mais alguma referência das possíveis funções, encontrei um [material no curso-r](material.curso-r.com/stringr/) muito bom tratando sobre este tema.
...
corpus <- Corpus(DataframeSource(data.frame(x)))
```
...
## Obtendo uma matriz de frquências a partir de um corpos
