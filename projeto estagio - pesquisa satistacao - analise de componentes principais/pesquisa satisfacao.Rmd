...
#Simulando os dados:
temp=matrix(0,nrow=99,ncol=ncol(dados))
temp=as.data.frame(temp)
for(i in 1:ncol(temp)){
temp[,i]=temp[,i]=sample(c(06.6667,3.3333,10),replace=T,size = 99)
}
names(temp)=str_c(str_sub(names(dados), end = 6),"_",str_sub(names(dados), start = -7))
...
Com uma base de dados referente a uma pesquisa de satisfação, foi realizada uma análise de componentes principais bem como o cálculo do coeficiente de correlação de spearman. Como os dados do estudo são de uso particular, uma amostra será simulada para que seja possível ilustrar quais os comandos utilizados em R para tais análises.
...
As interpretações pertinentes aos resultados calculados já foram realizadas e este documento possui a finalidade de ser um registro dos códigos em R utilizados para efetuar a análise de componentes principais, portanto a interpretação desses resultados fica como um exercício de estatística para o leitor.