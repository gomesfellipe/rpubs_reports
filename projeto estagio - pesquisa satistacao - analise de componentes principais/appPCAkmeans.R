...
#Pacotes utilizados
suppressMessages(library(shiny))        # Renderizar o aplicativo 
suppressMessages(library(ggplot2))      # Criar gráficos elegantes
suppressMessages(library(ggfortify))    # Criar gráficos de análises com ggplot
#suppressMessages(library(factoextra))  # Nao usa mais
suppressMessages(library(DT))           # Renderizar tabelas elegantes e práticas
suppressMessages(library(plotly))       # Transforma ggplot em interativo
suppressMessages(library(threejs))      # 3d interativo
suppressMessages(library(clValid)) # Validar clusters
...
server <- function(input, output) {                                  # Recebe os inputs da UI e calcula outputs 
  ...
  output$plot4 <- renderScatterplotThree({  
    ...
    scatterplot3js(as.matrix(cbind(acpcor$x[,1],acpcor$x[,2],acpcor$x[,3])),color = g$data$cluster,size = 0.5, flip.y=TRUE) 
  }) 
